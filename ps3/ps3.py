...
class RectangularRoom(object):
        ...
        if width > 0 and height > 0 and dirt_amount >= 0:
            ...
            for row in range(height):
                for col in range(width):
                    pos = (col, row)
                    self.tiles[pos] = dirt_amount
                    col += 1
                row += 1
        else:
            raise ValueError('invalid room parameter')
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        pos = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        if self.tiles[pos] <= capacity:
            self.tiles[pos] = 0
        else: 
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        if speed < 0 or capacity < 0:
            raise ValueError('invalid speed or capacity')
        self.speed = speed
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        xpos = self.width * random.random()
        ypos = self.height * random.random()
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        curPos = self.get_robot_position()
       
        newX = self.get_robot_position().get_x() + self.speed * math.cos(math.radians(90 - self.angle))
        newY = self.get_robot_position().get_y() + self.speed * math.sin(math.radians(90 - self.angle))
...
# print ('StandardRobot: ' + str(run_simulation(2, 1.0, 1, 20, 20, 3, 0.8, 50, StandardRobot)))
# print ('FaultyRobot: ' + str(run_simulation(2, 1.0, 1, 20, 20, 3, 0.8, 50, FaultyRobot)))
...
# for dim in [(10, 30), (20, 15), (25, 12), (50, 6)]:
#     print ('StandardRobot {}: '.format(dim) + str(run_simulation(2, 1.0, 1, dim[0], dim[1], 3, 0.8, 50, StandardRobot)))
#     print ('FaultyRobot {}: '.format(dim) + str(run_simulation(2, 1.0, 1, dim[0], dim[1], 3, 0.8, 50, FaultyRobot)))
